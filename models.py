...
class DenseModel(nn.Module):
    def __init__(self, input_size, outputs, dense_sizes):
        super(DenseModel, self).__init__()
        self.input_size = input_size
        self.dense_sizes = dense_sizes
        self.linear = nn.Linear(self.input_size, self.dense_sizes[0])
        self.denses = nn.ModuleList([nn.Linear(i, o) for i, o in zip(self.dense_sizes, self.dense_sizes[1:])])
        self.output = nn.Linear(self.dense_sizes[-1], outputs)
...
if __name__ == '__main__':
    ...
    n = np.arange(512).reshape(1,-1)
    t = from_numpy(n).float().unsqueeze(0)
    model = MSResNet(1, num_classes=1)
    print(model(t))
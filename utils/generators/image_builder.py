"""
This generator is for building channel_wise subtraction images using raw signals
consequently the input shape yielded by this generator is
(batch_size, channel, channel, 3) where all three channels are identical
"""
...
if __name__ == '__main__':
    ...
    train_path = Path(__file__).joinpath('..', '..', '..', 'files', 'dl_train_annotations.csv').resolve()
    val_path = Path(__file__).joinpath('..', '..', '..',  'files', 'dl_val_annotations.csv').resolve()
